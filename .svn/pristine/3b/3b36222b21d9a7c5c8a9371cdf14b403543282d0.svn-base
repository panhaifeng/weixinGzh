...
class Controller_Crm_ActionType extends Tmis_Controller {
	var $_modelExample;
	var $title = "行动方式";

	function Controller_Crm_ActionType() {
		$this->_modelExample = &FLEA::getSingleton('Model_Crm_ActionType');

		$this->fldMain = array(
        	'id'=>array('title'=>'','type'=>'hidden','value'=>''),
        	'name'=>array('title'=>'名称','type'=>'text','value'=>''),
        	'description'=>array('title'=>'描述','type'=>'text','value'=>''),
        	'sort'=>array('title'=>'排序','type'=>'text','value'=>''),
        );
	...
	////////////////////////////////标题
		...
		$pager =& new TMIS_Pager($this->_modelExample,$condition,'sort asc');
	...
	function actionSave() {
		//dump($_POST);exit;
		//新增时判断公司名和代码是否重复
		$str1="SELECT count(*) as cnt FROM `jichu_actiontype` where id<>'{$_POST['id']}' and name='{$_POST['name']}'";
