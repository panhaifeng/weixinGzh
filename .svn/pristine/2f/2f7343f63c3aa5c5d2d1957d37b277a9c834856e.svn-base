...
class Controller_Jichu_Sha extends Tmis_Controller {
	var $_modelExample;
	var $fldMain; 
	var $rules; //表单元素的验证规则
	// /构造函数
	function Controller_Jichu_Sha() {
	...
	function actionRight() {
		...
		$str = "select * from jichu_product where kind in('色纱','坯纱','其他','涤丝','氨纶')";
		...
		if (count($rowset) > 0) foreach($rowset as &$v) {
			...
			$v['price'] = "<input id='setPrice[]' size='5' name='setPrice[]' value='{$v['price']}'>
            <input id='id[]' size='5' name='id[]' type='hidden' value='{$v['id']}'>";
            if($v['editDate']=='0000-00-00 00:00:00'){
            	$v['editDate'] = '';
            }
		}
        ...
        $smarty->assign('sonTpl','Jichu/jsProduct.tpl');
