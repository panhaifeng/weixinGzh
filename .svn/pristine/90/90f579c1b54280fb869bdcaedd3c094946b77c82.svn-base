...
Class.pt.config = {
    ...
    callback:function(index,rowset) {}//返回对象后回调,一般选中某个数据后触发,2014-9-22 by jeff
};
...
Class.pt.openLayer = function(){
    ...
    //扩展callback方法,2014-9-22 by jeff,用在双击选择事件中
    //自定义回调函数
    //2014-9-22 by jeff
    layer.callback = function(index,rowset) {
        //执行config中的回调函数
        that.config.callback(index,rowset);
        layer.close(index);
    }
};
