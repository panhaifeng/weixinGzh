...
<table id="table_baseinfo">
	<tr>
		<td>出库日期：<input name="chukuDate" type="text" id="chukuDate"  value="{$arr_field_value.chukuDate|default:$smarty.now|date_format:'%Y-%m-%d'}" size="15" onClick="calendar()"></td>
		<td>领料部门：
		  <select name="depId" id="depId">{webcontrol type='TmisOptions' model='Jichu_Department' selected=$arr_field_value.depId}</select></td>
		<td>出库单号：<input name="chukuNum1" type="text" disabled id="chukuNum1" value="{$arr_field_value.chukuNum}" style="border:0px;" >
        			<input name="chukuNum" type="hidden" id="chukuNum" value="{$arr_field_value.chukuNum}" size="15" warning="请输入单号!" check="^\w+$" >
        </td>
		
	<tr>
	<td colspan="3">备注：<input name="orderMemo" type="text" id="orderMemo" value="{$arr_field_value.memo}" size="20"></td>
	</tr>
</table>
