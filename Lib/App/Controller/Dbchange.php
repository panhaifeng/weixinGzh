...
class Controller_Dbchange extends Tmis_Controller {
	...
	function actionShowLog() {
		...
		$arr = TMIS_Pager::getParamArray(array('dateFrom' => '',
				'dateTo' => ''
				));
		...
		$arr_field_info = array('fileName' => '文件名',
	...
	function actionGetPatchs4Up() {
		...
		if ($dh = opendir($this->patchPath)) {
			while (false !== ($file = readdir($dh))) {
				...
				$key = $temp[1].'_'.$temp1[0].'_'.$temp[0];
				$arrFile[$key] = $file;
				// if($file=='jeff_140617_0.txt') {
				// 	dump($temp);dump($arrFile);
				// }
				//dump($temp[1] . '_' . $temp1[0]);dump($file);
			};
			closedir($dh);
		} 
		// dump($arrFile);exit;
