...
class Controller_Crm_Industry extends Tmis_Controller {
	var $_modelExample;
	var $title = "客户行业";

	function Controller_Crm_Industry() {
		$this->_modelExample = & FLEA::getSingleton('Model_Crm_Industry');
	...
	function actionSave() {
		//dump($_POST);exit;
		//新增时判断是否重复
		$str1="SELECT count(*) as cnt FROM `jichu_industry` where id<>'{$_POST['id']}' and  name='{$_POST['name']}'";
